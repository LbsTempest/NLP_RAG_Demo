...
def build_semantic_index(documents, save_dir="merge_index", chunk_sizes=None):
    ...
    if not os.path.exists(save_dir):
        semantic_index = VectorStoreIndex(nodes, storage_context=storage_context)
        semantic_index.storage_context.persist(persist_dir=save_dir)
    else:
        semantic_index = load_index_from_storage(StorageContext.from_defaults(persist_dir=save_dir))
...
def main(args):
    ...
    index = build_semantic_index([document], save_dir=args.save_path)
    query_engine = index.as_query_engine()
    response = query_engine.query("When did Musk establish xAI")
    ...
    print('auto_merging_response:', response)
